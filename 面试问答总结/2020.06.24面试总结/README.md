...
-  这里只是沟通了TCP的三次握手通信，通信建立后就与面试官聊DNS解析的问题了
-  从TCP聊到了DNS如何进行解析，以及客户端拿到服务端文件后如何进行渲染
##### HTTP与HHTPS相关区别与联系
-  刚好前天和朋友聊天时说到了http和https默认端口的问题（运气好）
   1. 这里我先说了HTTP和HTTPS默认端口不同，一个80端口一个443
   2. HTTP是超文本传输协议，信息是明文传输，HTTPS是具有安全性的SSL加密传输协议
   3. HTTPS是通过SSL加密的传输协议（这里我讲到了自己建设个人网站时白嫖了阿里SSL证书）
   4. HTTP连接简单，是无状态的，HTTPS是SSL+HTTP构建的加密传输，身份认证的网络协议，比HTTP安全。
##### （笑）问下大家都问的，说说你在之前实习中都了解过哪些算法。
-  这里我讲了下市面经常说到的排序，说了几种排序的方案冒泡排和快排，并且分析下优劣
-  这里我也想到了之前我研究过的斐波那契数列，有点紧张没说（亏啊）
----
##### 一、解释下什么是事件代理
